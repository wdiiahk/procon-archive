...
struct Args
{
  arr<ll> edges1, edges2;

  Args() : edges1(C.N), edges2(C.N)
  {
    ll a, b;
    rep(C.M)
    {
      ...
      a--;
      b--;
      edges1.at(a).push_back(b);
      edges1.at(b).push_back(a);
    }
    ll c, d;
    rep(C.K)
    {
      ...
      c--;
      d--;
      edges2.at(c).push_back(d);
      edges2.at(d).push_back(c);
    }
    rep(i, C.N)
    {
      //out(i, ": friends ", edges1.at(i), " block ", edges2.at(i));
      Sort(edges1.at(i));
      Sort(edges2.at(i));
    }
...
struct Solver
{
  ...
  void solve()
  {
    ...
    each(g, uf.groups())
    {
      ...
      each(i, g)
      {
        ll n1 = args.edges1.at(i).size();
        ll n2(0);
        each(b, args.edges2.at(i))
        {
          if (uf.same(i, b))
          {
            n2++;
