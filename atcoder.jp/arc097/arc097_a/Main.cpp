...
struct Solver
{
  ...
  vll tmp;
  ll k;
  Solver() : ans(), sa(C.N + 1), rank(C.N + 1), tmp(C.N + 1), k(0)
  {
  }
  ...
  bool compare_sa(ll i, ll j)
  {
    ...
    {
      ll ri = i + k <= C.N ? rank.at(i + k) : -1;
      ll rj = j + k <= C.N ? rank.at(j + k) : -1;
      return ri < rj;
    }
  }
  ...
  void construct_sa()
  {
    // 最初の１文字、ランクは文字コードにすればよい
    rep(i, C.N + 1)
    {
      sa.at(i) = i;
      rank.at(i) = i < C.N ? C.S.at(i) : -1;
    }
    // k文字についてソートされているところから、２k文字でソートする
    for (k = 1; k <= C.N; k *= 2)
    {
      sort(all(sa), [&](ll i, ll j) {
        if (rank.at(i) != rank.at(j))
        {
          return rank.at(i) < rank.at(j);
        }
      ...
      rep(i, 1, C.N + 1)
      {
        tmp.at(sa.at(i)) = tmp.at(sa.at(i - 1));
        if (compare_sa(sa.at(i - 1), sa.at(i)))
        {
          tmp.at(sa.at(i)) += 1;
        }
      }
