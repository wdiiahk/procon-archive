...
struct Args
{
  ...
  Args() : edges(C.N)
  {
    ...
    rep(i, C.N - 1)
    {
      ...
      a--;
      b--;
      edges.at(a).emplace_back(b, i);
      edges.at(b).emplace_back(a, i);
    }
  }
...
struct Solver
{
  ...
  void dfs(ll x, ll p = -1, ll p_color = -1)
  {
    ll color(1);
    each(to, args.edges.at(x))
    {
      if (to.first == p)
      {
        continue;
      ...
      ans.at(to.second) = color;
      dfs(to.first, x, color);
      ++color;
    }
  }
  //
