...
/**
 * 円の列 C1,C2,...,CK は各 i に対し Ci+1 が Ci の strictly に内部にあるときサイズ K のターゲットであるという。 
 * すぬけ君は、円を N 個描いた。
 * i 番目の円は中心が (xi,0) であり、半径が ri である。
 * この中から円をいくつか選んでターゲットを作るとき、ターゲットの最大サイズを求めよ。 
 */
struct Args
{
  ll N;
...
public:
  ...
  ll LIS()
  {
    ...
    FORA(r, args.R)
    {
      auto it = lower_bound(ALL(dp), r.first);
      *it = r.first;
    }
