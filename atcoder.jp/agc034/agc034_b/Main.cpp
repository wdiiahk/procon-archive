...
struct Solver
{
  ...
  void solve()
  {
    vector<pair<char, ll>> datas;
    ll l = 0;
    while (l < C.N)
    {
      ll r = l + 1;
      char head = C.S.at(l);
      if (head == 'B' && r < C.N && C.S.at(r) == 'C')
      {
        r++;
        while (r + 1 < C.N && C.S.at(r) == 'B' && C.S.at(r + 1) == 'C')
        {
          r += 2;
        }
        datas.emplace_back('X', (r - l) / 2);
      }
      else
      {
        while (r < C.N && C.S.at(r) == head)
        {
          r++;
        }
        datas.emplace_back(head, r - l);
      }
      l = r;
    }
    ll pre(0);
