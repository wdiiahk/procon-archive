...
struct Solver
{
  ...
  ll bfs(ll root)
  {
    ...
    while (!pq.empty())
    {
      auto p = pq.top();
      pq.pop();
      FORA(to, args.g.at(p.second))
      {
        if (chmin(costs.at(to.second), costs.at(p.second) + to.first))
        {
          seen.at(to.second) = false;
        }
        if (to.second == root)
        {
          chmin(res, costs.at(p.second) + to.first);
        }
        if (seen.at(to.second))
        {
          continue;
        }
        seen.at(to.second) = true;
        if (res < costs.at(p.second) + to.first)
        {
          continue;
        }
        chmin(costs.at(to.second), costs.at(p.second) + to.first);
        pq.push({costs.at(to.second), to.second});
      }
    }
    return res;
  }
