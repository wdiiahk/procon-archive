...
struct Solver {
  ...
  */
  ...
  void solve() {
    vll rem(2, 0);
    FORA(a, args.a) { rem.at(a % 2)++; }
    ll n0(rem.at(0)), n1(rem.at(1));
    vll comb0(n0 + 1, 0), comb1(n1 + 1);
    comb0.at(0) = 1;
    REP(i, n0) { comb0.at(i + 1) = comb0.at(i) * (n0 - i) / (i + 1); }
    comb1.at(0) = 1;
    REP(i, n1) { comb1.at(i + 1) = comb1.at(i) * (n1 - i) / (i + 1); }
    /*
    ll f0(0);
    REP(i, n0 + 1) { f0 += comb0.at(i); }
    prp({f0, pow(2, n0)});
    */
    ans = pow(2, n0);
    if (C.P == 0) {
