...
struct Solver
{
  ...
  arr<ll> edges;
  vll dp;
  Solver() : ans(1), edges(C.N), dp(C.N, 3)
  {
  }
  //
  void solve()
  {
    rep(i, C.N - 1)
    {
      if (C.S1.at(i) == C.S2.at(i))
      {
        // 縦ブロック
        //edges.at(i).push_back(i + 1);
        edges.at(i + 1).push_back(i);
        if (C.S1.at(i + 1) != C.S2.at(i + 1))
        {
          // 次が横ブロック
          //edges.at(i).push_back(i + 2);
          edges.at(i + 2).push_back(i);
        }
      }
      else if (C.S1.at(i) == C.S1.at(i + 1))
      {
        //横ブロック上
        //edges.at(i).push_back(i + 1);
        edges.at(i + 1).push_back(i);
