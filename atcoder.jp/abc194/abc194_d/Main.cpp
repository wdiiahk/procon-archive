...
struct Solver
{
  ...
  void solve()
  {
    /*
    ll i = 1;
    dp.at(C.N - i) =  i / C.N * (dp.at(C.N - i)+ (C.N - i)/C.N * dp.at(C.N - i - 1)  + 1
    (C.N - i) / C.N * dp.at(C.N - i) = (C.N - i) / C.N * dp.at(C.N - i - 1) + 1
    */
    dp.at(0) = 0;
    rrep(i, 1, C.N)
    {
      dp.at(C.N - i) = dp.at(C.N - i - 1) + double(C.N) / (C.N - i);
    }
    ans = dp.at(C.N - 1);
  }
  //
