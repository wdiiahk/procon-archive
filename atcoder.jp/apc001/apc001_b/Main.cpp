...
struct Solver
{
  bool ans;
  vll c;
  Solver() : ans(false), c(C.N)
  {
    REP(i, C.N)
    {
      c.at(i) = args.a.at(i) - args.b.at(i);
    }
  }
  //
  void solve()
  {
    ll plusb = accumulate(ALL(c), 0LL, [](ll acc, ll x) {
      return (0 < x) ? acc + x : acc;
    });
    ll plusa = accumulate(ALL(c), 0LL, [](ll acc, ll x) {
      return (x < 0) ? acc + ((-x) / 2) : acc;
    });
    ans = (plusa >= plusb);
  }
  //
