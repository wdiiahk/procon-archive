...
public:
  Edge(ll _to, ll _t, ll _k) : to(_to), t(_t), k(_k)
  {
  }
};
...
struct Args
{
  ...
  Args() : edges(C.N)
  {
    rep(i, C.M)
    {
      ...
      a--;
      b--;
      edges.at(a).emplace_back(b, t, k);
      edges.at(b).emplace_back(a, t, k);
    }
  }
} args;
...
struct Solver
{
  ...
  void solve()
  {
    ...
    while (!q.empty())
    {
      ...
      for (auto e : args.edges.at(p.second))
      {
        ll next = (p.first + e.k - 1) / e.k * e.k + e.t;
        if (dp.at(e.to) < next)
        {
          continue;
        }
