...
struct Solver
{
  ...
  arr<int> edge;
  vec<pii> dp; // me, you
  Solver() : ans(0), edge(C.N), dp(C.N)
  {
    rep(i, C.N - 1)
    {
      edge.at(args.p.at(i)).emplace_back(i + 1);
    }
  }
  void dfs(int x, int p = -1)
  {
    ...
    each(to, edge.at(x))
    {
      ...
      dfs(to, x);
      int sz = dp.at(to).first + dp.at(to).second;
      if (sz % 2 == 0)
      {
        even.push_back(dp.at(to));
      }
      else
      {
        odd.push_back(dp.at(to));
      }
    }
    sort(all(even), [](pii &l, pii &r) {
      return l.second - l.first < r.second - r.first;
    });
    sort(all(odd), [](pii &l, pii &r) {
      return l.second - l.first < r.second - r.first;
    });
    ...
    while (!even.empty())
    {
      ...
      dp.at(x).first += p.first;
      dp.at(x).second += p.second;
      even.pop_back();
    }
    //
    int turn = 0;
