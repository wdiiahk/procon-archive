...
struct Solver
{
  ...
  Solver() : ans(0), counts(51), dp(51)
  {
    counts.at(0) = 1;
    dp.at(0) = {1, 0};
    rep(i, 1, 50 + 1)
    {
      dp.at(i).first = 2 * dp.at(i - 1).first + 1;
      dp.at(i).second = 2 * dp.at(i - 1).second + 2;
      counts.at(i) = dp.at(i).first + dp.at(i).second;
    }
  }
  ll dfs(ll n, ll x)
  {
    ...
    {
      return dp.at(n - 1).first + 1;
    }
    else if (x <= 2 + 2 * last)
    {
      return dp.at(n - 1).first + 1 + dfs(n - 1, x - 2 - last);
    }
    else
    {
      return 2 * dp.at(n - 1).first + 1;
    }
    return 0;
  }
