...
struct Solver
{
  ...
  void solve()
  {
    ...
    while (left < C.N)
    {
      ll right = left + 1;
      char t = C.S.at(left);
      while (right < C.N && t == C.S.at(right))
      {
        right++;
      }
      dp.emplace_back(t, right - left);
      left = right;
    }
    //
    ll T = dp.size();
    arr<ll> acc(T + 1, vll(27, 0));
    rrep(i, T)
    {
      rep(j, 27)
      {
      ...
      acc.at(i).at(0) += dp.at(i).second;
      char t = dp.at(i).first;
      acc.at(i).at(t - 'a' + 1) += dp.at(i).second;
    }
    pair<char, int> pre('?', T);
    rrep(i, T)
    {
      ...
      if (pre.first == t)
      {
        ans += acc.at(i + 1).at(0) - acc.at(pre.second).at(0);
      }
      else
      {
        ans += acc.at(i + 1).at(0);
      ...
      ll j = t - 'a' + 1;
      ans -= acc.at(i + 1).at(j) - acc.at(pre.second).at(j);
      pre = {dp.at(i).first, i};
    }
  }
  //
