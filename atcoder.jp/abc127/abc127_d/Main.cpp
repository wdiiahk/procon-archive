...
struct Args {
  ...
  Args() : a(C.N), p(C.M) {
    ...
    sort(ALL(a));
    REP(i, C.M) { cin >> p.at(i).first >> p.at(i).second; }
    sort(ALL(p), [&](P &l, P &r) { return l.second > r.second; });
  }
} args;
...
struct Solver {
  ...
  void solve() {
    ll cur(0);
    FORA(p, args.p) {
      ll from(cur), to(cur + p.first);
      FOR(i, from, to) {
        if (C.N <= i) break;
        if (p.second < args.a.at(i)) {
          cur = C.N;
          break;
        }
        args.a.at(cur) = p.second;
        ++cur;
      }
      if (C.N <= cur) break;
    }
