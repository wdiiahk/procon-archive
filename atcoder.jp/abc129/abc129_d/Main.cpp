  ...
  void calc_next_idx_for_width()
  {
    REP(h, C.H)
    {
      REPD(w, C.W)
      {
        ...
        {
          dp1.at(h).at(w) = dp1.at(h).at(w + 1);
        }
      }
    }
  }
  void calc_counts_for_width()
  {
    REP(i, C.H)
    {
      ll pre(-1);
      REP(j, C.W + 1)
      {
        ll next = dp1.at(i).at(j);
        if (next == j)
        {
          pre = j;
        }
        dp1.at(i).at(j) = (next - pre - 1);
      }
    }
  }
  void calc_next_idx_for_hight()
  {
    REP(w, C.W)
    {
      REPD(h, C.H)
      {
        ...
        {
          dp2.at(h).at(w) = dp2.at(h + 1).at(w);
        }
      }
    }
  }
  void calc_counts_for_hight()
  {
    REP(j, C.W)
    {
      ll pre(-1);
      REP(i, C.H + 1)
      {
        ll next = dp2.at(i).at(j);
        if (next == i)
        {
          pre = i;
        }
        dp2.at(i).at(j) = (next - pre - 1);
      }
    }
  }

  Solver() : ans(1), dp1(C.H + 1, vll(C.W + 1, C.W)), dp2(C.H + 1, vll(C.W + 1, C.H))
  {
    calc_next_idx_for_width();
    calc_counts_for_width();
    calc_next_idx_for_hight();
    calc_counts_for_hight();
  }
  //
  void solve()
  {
    REP(i, C.H)
    {
      REP(j, C.W)
      {
        chmax(ans, dp1.at(i).at(j) + dp2.at(i).at(j));
      }
    }
    --ans;
  }
