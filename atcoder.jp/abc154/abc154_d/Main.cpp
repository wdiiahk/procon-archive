...
struct Solver {
  ...
  //
  void solve() {
    vector<double> dp(C.PMAX + 1, 0);
    FOR(i, 1, C.PMAX + 1) { dp.at(i) = double(i + 1) / 2; }
    vector<double> acc(C.N + 1, 0);
    REP(i, C.N) { acc.at(i + 1) = acc.at(i) + dp.at(args.a.at(i)); }
    //
    FOR(i, C.K, C.N + 1) {
      double p = acc.at(i) - acc.at(i - C.K);
      chmax(ans, p);
    }
  }
