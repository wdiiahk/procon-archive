...
struct Args
{
  ...
  Args() : edges(C.N)
  {
    ...
    rep(i, C.N - 1)
    {
      ...
      U--;
      V--;
      edges.at(U).emplace_back(V, W);
      edges.at(V).emplace_back(U, W);
    }
  }
} args;
...
struct Solver
{
  vll ans;
  atcoder::dsu uf;
  Solver() : ans(C.N, 0), uf(C.N + 2)
  {
  }
  void dfs(ll x, ll p = -1, bool black = false)
  {
    uf.merge(x, (black) ? C.N : C.N + 1);
    each(e, args.edges.at(x))
    {
      if (e.first == p)
      {
        continue;
