...
struct Solver
{
  ...
  void encode()
  {
    ...
    while (left < C.N)
    {
      ll right = left;
      char c = C.S.at(left);
      while (right < C.N && c == C.S.at(right))
      {
        right++;
      }
      enc.emplace_back(c, right - left);
      left = right;
    }
    if (enc.back().first == '0')
    {
  ...
  void solve()
  {
    ...
    rep(i, T)
    {
      acc.at(i + 1) = acc.at(i) + enc.at(i).second;
    }

    rep(left, T)
    {
      ll right = T;
      if (enc.at(left).first == '1')
      {
        chmin(right, left + 2 * C.K + 1);
      }
      else
      {
        continue;
        //chmin(right, left + 2 * C.K);
      }
      chmax(ans, acc.at(right) - acc.at(left));
    }
  }
  //
  void output()
