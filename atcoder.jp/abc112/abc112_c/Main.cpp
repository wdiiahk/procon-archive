...
vector<ll> divisor(ull x)
{
  ...
  rrep(ans.size() - (ans.back() * ans.back() == ll(x))) ans.push_back(x / ans[i]);
  return ans;
}
template <class T>
...
struct Solver
{
  ...
  ll f(vll &a, pll &b)
  {
    return a.at(2) + abs(a.at(0) - b.first) + abs(a.at(1) - b.second);
  }

  bool check(vll &a, pll &b, ll h)
  {
    return h - abs(a.at(0) - b.first) - abs(a.at(1) - b.second) <= 0;
  }
  ...
  void solve()
  {
    ...
    vll top(args.a.at(0));
    queue<pll> q;
    q.push({top.at(0), top.at(1)});
    arr<bool> seen(101, vector<bool>(101, false));
    seen.at(top.at(0)).at(top.at(1)) = true;
    while (!q.empty())
    {
      auto p = q.front();
      q.pop();
      ...
      if (ok)
      {
        ans = vll({p.first, p.second, h});
        return;
      }
      //
      rep(i, 4)
      {
        pll next(p.first + four[i], p.second + four[i + 1]);
        if (next.first < 0 || 100 < next.first)
        {
          continue;
        }
        if (next.second < 0 || 100 < next.second)
        {
          continue;
        }
        if (seen.at(next.first).at(next.second))
        {
          continue;
        }
        seen.at(next.first).at(next.second) = true;
        q.push(next);
      }
    }
  }
