...
struct Solver
{
  ...
  Solver() : ans(0), edge(C.N), order(C.N, -1), low(C.N), cmp(C.N, -1)
  {
    FORA(p, args.a)
    {
      edge.at(p.first - 1).push_back(p.second - 1);
      edge.at(p.second - 1).push_back(p.first - 1);
    }
  }
  void dfs(ll v, ll p, ll &k)
  {
    ...
    FORA(to, edge.at(v))
    {
      ...
      {
        dfs(to, v, k);
        chmin(low.at(v), low.at(to));
        if (order.at(v) < low.at(to))
        {
